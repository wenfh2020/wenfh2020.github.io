...
2. client 生成一个随机数 **a 作为私钥**，通过原理 `Q = kP` 进行椭圆曲线点运算 `A = aG`，获得 A 点，**A 点作为 client 的公钥**。
3. client 将椭圆曲线 G 点等公开信息和它的公钥 A 发送给 Bob。
4. server 生成一个随机数 **b 作为私钥**，通过原理 `Q = kP`  进行椭圆曲线点运算 `B = bG`，获得 B 点，**B 点作为 server 的公钥**。
...
结合下图理解 TLS 握手的各个阶段。握手原理是双方通过互换信息，协商出共享的会话密钥。

* 客户端随机数，服务端随机数都是明文，只有 `预主密钥` 是通过椭圆曲线的点算法运算出来的。

<div align=center><img src="/images/2023/2023-10-14-16-54-31.png" data-action="zoom"></div>
