...
<style> table th:nth-of-type(5) { width: 50px; } </style>

|序号|方向|握手协议|描述|抓包|
|:----:|:--------------:|:--:|:--|:--:|
|<span style="display:inline-block;width:40px">1</span>|<span style="display:inline-block;width:50px">C --> S </span>|<span style="display:inline-block;width:120px">Client Hello</span>|客户端发送它支持的 `TLS 版本`，`加密套件列表`，`客户端随机数` 给服务端。|<img src="/images/2023/2023-10-15-19-42-54.png" width="50%" data-action="zoom"/>|
|2|S --> C|Server Hello|服务端发送它选择的 `TLS 版本`，`加密套件`，`服务端随机数` 给客户端。|<img src="/images/2023/2023-10-15-19-58-18.png" width="50%" data-action="zoom"/><img src="/images/2023/2023-10-11-14-17-55.png" width="50%" data-action="zoom"/>|
|3|S --> C|Certificate|服务端发送 CA 证书（`公钥` + 证书持有者等信息）给客户端。|<img src="/images/2023/2023-10-15-20-03-04.png" width="50%" data-action="zoom"/>|
|4|S --> C|Server Key Exchange|1. 服务端生成 `椭圆曲线私钥` ==> 生成 `椭圆曲线公钥` ==> 服务端的 RSA 私钥实现 `椭圆曲线公钥签名`。<br/>2. 服务端发送 `椭圆曲线算法信息`，`（服务端的）椭圆曲线公钥`，`（服务端的）椭圆曲线公钥签名` 给客户端。|<img src="/images/2023/2023-10-16-03-34-08.png" width="50%" data-action="zoom"/>|
|5|S --> C|Server Hello Done|服务端发送确认给客户端，已完成 Hello 阶段流程。|<img src="/images/2023/2023-10-16-03-38-25.png" width="50%" data-action="zoom"/>|
|6|C --> S|Client Key Exchange|客户端生成 `椭圆曲线公钥`，并将其发送给服务端。<br>1. 客户端接收到证书后，通过本地系统的 `证书链` 验证该证书是否合法。<br/>2. 客户端通过证书公钥解签 `（服务端的）椭圆曲线公钥`，确认该数据的完整性和安全性。<br/>3. 客户端生成 `椭圆曲线私钥` ==> 生成 `椭圆曲线公钥`。<br/>4. 客户端使用服务端的 RSA 公钥加密它生成的 `椭圆曲线公钥`，并将其发送给服务端。|<img src="/images/2023/2023-10-16-04-03-56.png" width="50%" data-action="zoom"/>|
|7|C --> S|Change Cipher Spec|客户端通知服务端，确认握手过程中的加密算法和密钥已经生效。|
...
<br/>3. 至此 **（服务端的）椭圆曲线公钥** 和 **（客户端的）椭圆曲线私钥** 通过椭圆曲线算法算出对应的点 (X1，Y1)，取 X1 坐标。<br/>4. **（客户端的）第 1 随机数** +**（服务端的）第 2 随机数** + **X1 坐标数值** => `客户端的会话秘钥`。<br/>5. 服务端接收到客户端发送的 **（客户端的）椭圆曲线私钥** 后，也可以通过已有的 **服务端椭圆曲线私钥** 和 **客户端的椭圆曲线公钥** 根据椭圆曲线算法算出对应的点 (X2，Y2)，取 X2 坐标。<br/>6. **（客户端的）第 1 随机数** +**（服务端的）第 2 随机数** + **X2 坐标数值** => `服务端的会话秘钥`。<br/>7. 因为 X2 == X1，所以 `客户端的会话秘钥` == `服务端的会话秘钥`。<br/>8. 至此双方的对称密钥协商成功。
