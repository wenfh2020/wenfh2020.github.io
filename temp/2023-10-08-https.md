...
2. 协商加密：双方通过 ECDHE 非对称加密算法，协商出相同的 `对称会话秘钥` 进行内容对称加密通信，避免传输会话秘钥被中间人窃取。
...
1. 客户端和服务器各自选择一个私钥，并使用椭圆曲线算法生成对应的公钥。
2. 在握手阶段，客户端和服务器互相 `交换公钥`。
3. 客户端和服务器各自使用 `对方的公钥` 和 `自己的私钥`，通过椭圆曲线的点运算生成一个 `共享密钥`。由于椭圆曲线的数学性质，这个共享密钥在双方都知道，但是不能被第三方计算出来。
...
通信双方通过 `非对称加密` 方式，协商出一个 `对称的会话秘钥`，然后通过该秘钥实现双向数据保密传输，防止泄漏。

下面将通过下图详细理解 TLS 握手的各个阶段。
...
|6|C --> S|Client Key Exchange|客户端将它生成的 **（客户端的）临时公钥** 发送给服务端。<br/> 1. 客户端生成临时随机数 ==> 生成椭圆曲线算法 **临时私钥** ==> 生成 **临时公钥**。<br/>2. 至此 **（服务端的）临时公钥** 和 **（客户端的）临时私钥** 通过椭圆曲线算法算出对应的点 (X1，Y1)，取 X1 坐标。<br/>3. **（客户端的）第 1 随机数** +**（服务端的）第 2 随机数** + **X1 坐标数值** => `客户端的会话秘钥`。<br/>4. 服务端接收到客户端发送的 **（客户端的）临时私钥** 后，也可以通过已有的 **服务端临时私钥** 和 **客户端的临时公钥** 根据椭圆曲线算法算出对应的点 (X2，Y2)，取 X2 坐标。<br/>5. **（客户端的）第 1 随机数** +**（服务端的）第 2 随机数** + **X2 坐标数值** => `服务端的会话秘钥`。<br/>6. 因为 X2 == X1，所以 `客户端的会话秘钥` == `服务端的会话秘钥`。<br/>7. 至此双方的对称密钥协商成功。|
