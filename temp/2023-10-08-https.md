...
1. 客户端和服务器各自选择一个 `临时私钥`（随机数），并使用椭圆曲线算法生成对应的 `临时公钥`。
2. 在握手阶段，客户端和服务器互相交换 `临时公钥`。
3. 客户端和服务器各自使用 `对方的临时公钥` 和 `自己的临时私钥`，通过椭圆曲线的点运算生成一个 `共享密钥`。由于椭圆曲线的数学性质，这个共享密钥在双方都知道，但是不能被第三方计算出来。(共享秘钥并非通过传输手段获得，避免中间人截取。)
...
参考下图，Alice 和 Bob 通过椭圆曲线算法协商出来一个共享密钥。

1. 椭圆曲线 **G 点** 等一些信息是公开的。
2. Alice 生成一个随机数 **a 作为私钥**，通过原理 `Q = kP` 进行椭圆曲线点运算 `A = aG`，获得 A 点，**A 点作为 Alice 的公钥**。
3. Alice 将椭圆曲线 G 点等公开信息和它的公钥 A 发送给 Bob。
4. Bob 生成一个随机数 **b 作为私钥**，通过原理 `Q = kP`  进行椭圆曲线点运算 `B = bG`，获得 B 点，**B 点作为 Bob 的公钥**。
5. Bob 将 B 点发送给 Alice。
6. Bob 将 自己的私钥 b 和 Alice 发送的公钥 A 点，进行点运算获得新秘钥：新秘钥（点）= bA = b(aG)。
7. Alice 将自己的私钥 a 和 Bob 发送的公钥 B 点，进行点运算获得新秘钥：新秘钥（点）= aB = a(bG)。
8. 此时双方都生成了新秘钥，根据乘法交换律 b(aG) = a(bG) 推出 bA = aB = 新秘钥（点），也就是说双方的新秘钥是一致的，这就是需要计算的 `共享秘钥`。

因为双方发送的是自己的公钥，还有 G 点等公开信息，回顾上文 `Q = kP`，知道 Q 点和 P 点，计算出 k 是十分困难的，换句话说，知道了公钥和椭圆曲线基点等公开信息，要破解出私钥 k 是非常困难的。

<div align=center><img src="/images/2023/2023-10-13-16-34-07.png" data-action="zoom"/></div>

> 图片来源：[公钥加密技术ECC椭圆曲线](https://www.bilibili.com/video/BV1BY411M74G/?spm_id_from=333.337.search-card.all.click&vd_source=a2a56cf0a934465d3945d595a71e68dc)
