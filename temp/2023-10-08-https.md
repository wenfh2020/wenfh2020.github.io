...
2. 在握手阶段，客户端和服务器互相交换公钥。这个过程通常在 ClientHello 和 ServerHello 消息中完成（抓包可以看到详细信息）。
...
* 这里用了 ECDH 秘钥交换，你都没结合 Client Key Exchange 报文里的 pubkey 是作为 ECDH 的参数传过去的。

* 预主秘钥相当于 ECDH 秘钥交换中的生成的随机数 n，n 点 G 得到 ECDH 公钥，再将 ECDH 公钥使用证书中服务器的私钥签名防止被篡改。

* 服务器收到后使用公钥解签，得到客户端的 ECDH 公钥与自己的 ECDH 私钥一乘才是 ECDH 交换的秘钥。
再将两个随机数和 ECDH 交换的秘钥作为对称加密的秘钥使用吧。
...
* **TLS 握手**：双方保密通信，避免秘钥传递被中间人截取，不能直接传递秘钥；双方的秘钥获得方式是：双方通过非对称加解密方式，协商出一个对称密钥。
...
通信双方通过 `非对称加密` 方式，协商出一个 `对称的会话秘钥`，然后通过该秘钥实现双向数据保密传输，防止泄漏。下面将通过下图详细理解 TLS 握手的各个阶段。

<div align=center><img src="/images/2023/2023-10-10-09-45-04.png" data-action="zoom"/></div>

|序号|方向|阶段|描述|
|:----:|:--------------:|:--:|:--|
|<span style="display:inline-block;width:40px">1</span>|<span style="display:inline-block;width:50px">C --> S </span>|<span style="display:inline-block;width:200px">Client Hello</span>|客客户端发送它支持的 **TLS 版本**，**加密套件列表**，**第 1 随机数** 给服务端。|
|2|S --> C|Server Hello|服务端发送它选择的 **TLS 版本**，**加密套件**，**第 2 随机数** 给客户端。|
|3|S --> C|Certificate|服务端发送 CA 证书（`公钥` + 其它信息）给客户端，客户端接收到证书后，会通过本地系统的 **证书链** 验证该证书是否合法。|
|4|S --> C|Server Key Exchange|服务端会发送 **椭圆曲线点乘算法信息**，**（服务端的）临时公钥**，**（服务端的）临时公钥签名**（服务端的 RSA 私钥实现签名，非临时私钥）给客户端，客户端接收到相关数据后，通过证书公钥解签服务端的 **临时公钥**，确认数据的完整性和安全性。|
|5|S --> C|Server Hello Done|服务端发送确认给客户端，已完成 Hello 阶段流程。|
|6|C --> S|Client Key Exchange|客户端将它生成的 **（客户端的）临时公钥** 发送给服务端。<br/>* 客户端生成临时随机数 ==> 生成椭圆曲线算法 **临时私钥** ==> 生成 **临时公钥**。<br/>* 至此 **（服务端的）临时公钥** 和 **（客户端的）临时私钥** 通过椭圆曲线算法算出对应的点 (X1，Y1)，取 X1 坐标。<br/>**（客户端的）第 1 随机数** +**（服务端的）第 2 随机数** + **X1 坐标数值** => `客户端的会话秘钥`。<br/>* 服务端接收到客户端发送的 **（客户端的）临时私钥** 后，也可以通过已有的 **服务端临时私钥** 和 **客户端的临时公钥** 根据椭圆曲线算法算出对应的点 (X2，Y2)，取 X2 坐标。<br/>* **（客户端的）第 1 随机数** +**（服务端的）第 2 随机数** + **X2 坐标数值** => `服务端的会话秘钥`。<br/>* 因为 X2 == X1，所以 `客户端的会话秘钥` == `服务端的会话秘钥`。<br/>* 至此双方的对称密钥协商成功。|
...
#### 2.2.4. Server key Exchange，Server Hello Done

TLS 的 Server Key Exchange 阶段是在 TLS 握手过程中的一个步骤，用于服务器向客户端发送 **临时公钥** 和 **签名**，以确保通信的安全性。

具体原理如下：

1. 服务器选择一个 **临时私钥**，并生成对应的 **临时公钥**。
2. 服务器使用自己的私钥对 **临时公钥** 进行（RSA 算法）签名，生成签名值。
3. 服务器将 **临时公钥** 和 **签名值** 发送给客户端。
4. 客户端收到服务器的临时公钥和签名值后，使用服务器的 **公钥** 验证签名的有效性。
5. 如果签名有效，客户端使用服务器的临时公钥进行密钥交换。

> 文字来源：ChatGPT

<div align=center><img src="/images/2023/2023-10-08-16-33-00.png" width="70%"  data-action="zoom"/></div>

#### 2.2.5. Client Key Exchange, Change Cipher Spec, Encrypted Handshake Message
