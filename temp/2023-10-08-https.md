...
通过 wireshark 抓取 HTTPS 通信包，理解 TLS 的安全通信流程。
...
我们可以通过浏览器访问某些 HTTPS 域名，通过抓包工具（wireshark），抓包分析 TLS 的工作流程。
...
TLS 目的是，通信双方通过 `非对称加密` 方式，协商出一个 `对称的会话秘钥`。
...
<div align=center><img src="/images/2023/2023-10-09-16-15-47.png" data-action="zoom"/></div>
...
|7|C --> S|Change Cipher Spec|客户端和服务器都发送该消息，表示之后的通信将使用新的加密算法和密钥。|
...
|10|S --> C|Change Cipher Spec|客户端和服务器再次发送该消息，确认握手过程中的加密算法和密钥已经生效。|
...
* 证书来源。证书来源渠道很多，普通证书可以通过 openssl 本地生成，而由权威的证书颁发机构 CA（Certificate Authority）签发的数字证书须要通过相关平台付费购买或者免费申请。
...
|配置项|描述|
|:---|:---|
|ssl_certificate|指定服务器证书的路径。服务器证书是由CA机构签发的，用于 **验证服务器的身份**。它包含了服务器的公钥和其他相关信息。|
