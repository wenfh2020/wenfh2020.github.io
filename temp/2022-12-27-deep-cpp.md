---
layout: post
title:  "[c++] 深度探索 C++ 多态工作原理"
categories: c/c++
tags: stl sort
author: wenfh2020
---

最近翻阅侯捷先生的两本书：《深度探索 C++ 对象模型》和《C++ 虚拟与多态》，对 CPP 的多态工作原理，又多了一些理解。

对象内存布局、虚指针、虚函数、虚表 ——能整明白这几个点的关系，应该可以大概理解多态的工作原理了。
...
多态的价值一般体现在有继承关系的类结构，但是简单的无继承关系的结构，也能体现出多态工作原理，我们通过下面例子进行剖析多态的工作原理。
...
* 反汇编源码。反汇编查看 main 函数源码，观察程序是如何链路到虚函数的函数地址的，并将其放进 rax 寄存器，提供 call 命令调用。
