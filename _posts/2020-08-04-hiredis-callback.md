...
![hiredis + libev 工作流程](/images/2020-08-05-15-38-08.png){:data-action="zoom"}

首先通信链接 socket 被设置为非阻塞的。

hiredis 不是一个独立实现的 c - client，它需要基于第三方库。例如它结合 `libev`，Linux 系统下，libev 默认用 epoll 多路复用技术处理处理读写事件。用户调用 hiredis 的发送数据接口，并不是马上将数据发送出去，而是先保存在发送缓冲区，然后当 libev 触发写事件，才会将发送缓冲区的数据发送出去。
