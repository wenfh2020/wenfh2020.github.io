...
在 C++11 之前，我们通常通过复制构造函数来复制对象。但是这种方式可能会导致资源浪费。

通过使用右值引用，我们可以直接将资源从一个对象移动到另一个对象，无需进行复制，从而提高了效率。
...
因为资源的移动涉及到源对象和目标对象的资源重置，有一定的危险性，特别是源对象被移动处理后被再次使用是非常危险的。
