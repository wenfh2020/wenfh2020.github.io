...
rdb 文件，可以看作是 redis 数据库文件，是 redis 持久化方式之一。
...
从默认配置看，rdb 持久化不是实时的。存盘时间间隔，最迟 900 秒存盘一次，最快 60 秒存盘一次。所以用 rdb 存盘丢失数据风险比较大。
...
int serverCron(struct aeEventLoop *eventLoop, long long id, void *clientData) {
    ...
    ...
    // 我们在执行 BGSAVE 命令时，因为当时有其它子进程正在进行工作，所以该命令被安排延后处理。
