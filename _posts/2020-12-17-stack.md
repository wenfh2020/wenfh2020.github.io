...
categories: 技术
tags: stack asm lldb registers
author: wenfh2020
---

为了搞明白协程切换原理，重拾已经还给老师的汇编知识。

通过 `lldb` 调试器，在 64 位的 MacOS 机器上调试测试程序。通过查看相关数据（程序汇编编码，寄存器数据，对应数据内存地址），去理解运行时栈的内存布局。
...
源码被编译后生成的二进制文件被载入内存后，CPU，内存，磁盘等重要设备相互协调工作，完成程序的指令。
...
程序被载入内存后，以进程方式运行，进程在系统用户空间运行，用户进程一般情况下不能直接访问物理内存，它通过虚拟内存进行寻址。下图是进程运行时栈空间在(32 bit)虚拟内存的内存分布位置。栈空间的增长特点：数据从高地址向低地址压栈。
...
下图是函数运行时，函数相关数据在内存上的布局情况。一般情况下，每个函数在运行时，系统会为它分配临时栈内存。

* 函数运行时，相关数据除了保存在`内存`（栈内存空间），但也需要通过`寄存器`传递或保存数据。
* 栈内存的生命期与当前函数的生命期一样。
* 栈大小一般情况下是（%ebp/%rbp）函数基地址和 (%esp/%rsp) 函数栈顶地址之间的内存块。
  > 有时候不一定需要函数栈顶地址，因为函数栈顶地址可以通过函数基地址结合变量等数据计算出来。
* 栈内存保存了：函数传参数数据，函数内部临时变量数据，函数基地址，函数调用者运行的指令地址等等。

<div align=center><img src="/images/2020-12-16-16-24-24.png" data-action="zoom"/></div>

> 图片来源：[《Libco 协程栈的切换理解》](https://vinsflyfish.github.io/posts/thinking-in-libco/)

---

## 2. lldb 源码调试

下面将通过简单的 c 语言测试源码，并通过 lldb 调试器，分步调试汇编，搞清楚函数运行时栈内存的分配原理。
...
| 读内存                           | me read <br/> me read -fx -s4 -c1 \<addr\> |

---

#### 2.4.2. 调试流程

下面是 lldb 调试流程，可以通过 `si` 命令，单步调试汇编指令，查看程序运行时栈内存数据，寄存器数据，从而熟悉栈的内存布局。

> 本次调试是在 64 位 MacOS 下运行的，调试方法与 Linux 上使用 Gdb 相差不大。有兴趣的朋友可以自己写 demo，动手调试一下。
