...
当用户关注的 fd 事件发生时，et 模式，只通知用户一次，不管这个事件是否已经被用户处理完毕，直到该事件再次发生，或者用户通过 `epoll_ctl` 重新关注该 fd 对应的事件（参考下面源码）。而 lt 模式，会不停地通知用户，直到用户把事件处理完毕。
...
这个道理有点像排队打饭，一个队列上，有一个同学要打包两份饭，如果每次只能打包一份，lt 模式就是，这个同学打包了一份后，重新回去排队，再打一份。et 模式是，这个同学先打包一份，然后跑去吃掉了，再回来排队，显然这个排队的结果不一样。

---

不要小看这个处理时序，在高并发系统里，系统可能负载比较高的，每个后来者都希望自己的事件快点被处理，而 et 模式可以提高这个被处理的速度。

同时如果我们仔细观察服务程序的 listen 接口，它有一个 backlog 参数，代表 listen socket 存储就绪链接的完全队列的长度，这说明队列是有限制的，不是无限的，当它满了会返回错误给客户端，所以完全队列的数据当然越快得到处理越好。
...
1. epoll 的 lt / et 模式实现逻辑主要在内核的 epoll_wait 里。
2. epoll_wait 的关键数据结构是事件就绪队列。
3. lt / et 模式区别主要有：通知方式，新事件快速处理，避免类似惊群问题。
4. lt / et 模式的设计意图在于：事件处理的紧急程度。举个 🌰：假如你有急事打电话找人，如果对方一直不接，那你只有一直打，打爆他的电话，直到他接为止，这就是 lt 模式；如果你纯粹是无聊想打发一下时间联系对方，那打过去对方不接，那就等有空再打~~~ 6666。

## 4. 后记

使用 epoll 已经很长一段时间了，一直困扰着 lt / et 模式的区别，直到深入阅读内核源码后，才慢慢地理解它的工作原理，其实逻辑不是想象的那么复杂，可见阅读内核源码的重要性！
