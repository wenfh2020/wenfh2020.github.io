...
lldb 是 MacOS 下的调试器，类似 gdb，下面记录一下比较少用的操作。

```shell
# 带参数调试命令。
lldb kimserver -- config.json
# 进入 lldb 调试后，环境变量设置。
env DYLD_LIBRARY_PATH=&DYLD_LIBRARY_PATH:/usr/local/lib/mariadb/
# 设置断点
b zk_mgr.cpp : 61
# 监听指针的变化。
watchpoint set expression -- <ptr>
```
