...
* `maxmemory` 设置其实是一个学问，到底应该设置多少，才比较合理。很多人建议是物理内存大小的一半，因为 redis 会 fork 子进程，在 linux 系统中虽然父子进程有 'copy-on-write' 模式，redis 也尽量避免子进程工作过程中修改数据，说不定哪个操作会大量修改内存；`maxmemory` 限制的内存并不包括 `aof` 缓存和主从同步积压缓冲区部分内存，而且我们的机器有时不是只跑 redis 一个进程的，系统其它进程也要使用内存。
