...
前面截取了很多代码片段，为了有一个连续的印象，还是把核心源码和相关注释贴一下。
...
* redis 从 2.x 到 5.x，一直不停地改进迭代，redis 作者精益求精的精神值得我们学习。
* 采样近似淘汰策略，巧妙避免了维护额外的数据结构，达到差不多的效果，这个思路独具匠心。
* 采样算法，根据样本的 idle 值进行数据淘汰，所以当我们采用一种采样算法时，不要密集地设置大量相似的 idle 数据，否则效率也是很低的。
* `maxmemory` 设置其实是一个学问，到底应该设置多少，才比较合理。很多人建议是物理内存大小的一半，因为 redis 会 fork 子进程，在 linux 系统中虽然父子进程有 'copy-on-write' 模式，redis 也尽量避免子进程工作过程中修改数据，说不定哪个地方会漏掉大量修改内存也不一定。而且 `maxmemory` 限制的内存并不包括 `aof` 缓存和主从同步积压缓冲区部分内存。
