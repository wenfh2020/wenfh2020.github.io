...
![对象关系](/images/2020-02-20-14-09-19.png)
...
为了增加内存的使用，每个内存对象进可能共享，被共享使用的内存对象，每次被引用会增加计数，对象被回收计数会递减。回收对象发现计数为 1，就会直接被删除。
...
对象实现可以通过调试，看看实现逻辑。刚看完[整数集合](https://wenfh2020.github.io/2020/02/04/redis-inset/)，可以跟踪下实现流程。
